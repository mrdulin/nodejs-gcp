...
exports.createAdGroup = async function createAdGroup(data, context, callback) {
  ...
  const adGroup = await createAdGroupHTTPOperation(traceContext);
  console.log('adGroup: ', adGroup);
  await pubsub.topic('adGroupCreated').publish(Buffer.from(jsonString));
  callback();
};
...
function createAdGroupHTTPOperation(traceContext) {
  traceContext = generateTraceContext(traceContext);
  const body = { operation: 'create ad group' };
